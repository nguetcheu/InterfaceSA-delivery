...
@Component({
  selector: 'app-admin-pie-standby',
  templateUrl: './admin-pie-standby.component.html',
  styleUrls: ['./admin-pie-standby.component.scss'],
})
export class AdminPieStandbyComponent implements OnInit {
  Chart!: any;
  ...
  ngOnInit(): void {
    this.adminService.getCommandes().subscribe((data: any[]) => {
      // Filtrer les commandes ayant le statut "enAttente"
      const commandesValides = data.filter(
        (commande) => commande.statut === 'enAttente'
      );
      ...
      const sortedClients = Object.keys(aggregatedData).sort(
        (a, b) => aggregatedData[b] - aggregatedData[a]
      );

      // Créer le graphique en forme de camembert (pie chart)
      this.createPieChart(
        sortedClients.map((client) => aggregatedData[client]),
        sortedClients
      );
    });
  }
  ...
  createPieChart(data: number[], labels: string[]) {
    const ctx = document.getElementById('clientAttenteChart') as HTMLCanvasElement;
